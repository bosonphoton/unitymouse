...
class Policy:
	def __init__(self,num_states,num_actions, policy_file='out.policy'):
		#f=None
		try:
			f = open(policy_file, 'r')
		#	lines = f.readlines()
		except:
			print('\nError: unable to open file: ' + policy_file)
...
class Solver:
	def __init__(self):
		...
		path = '/home/'+getpass.getuser()+'/chelsea/dialogue_eye_gaze_ws/src/move_robot_gaze/scripts/pomdp/appl/src/pomdpsol'
		if os.path.exists(path):
			subprocess.check_output(path + ' program.pomdp --timeout 60 --output out.policy', shell=True)
