...
public class StaticAruco : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float red_dist, green_dist, blue_dist;
        float thres = 0.2f;

        red_dist = Vector3.Distance(initializedprojectedred.transform.position, initializedred.transform.position);
        green_dist = Vector3.Distance(initializedprojectedgreen.transform.position, initializedgreen.transform.position);
        blue_dist = Vector3.Distance(initializedprojectedblue.transform.position, initializedblue.transform.position);

        if (red_dist < thres)
        {
            Rigidbody rb;
            rb = initializedprojectedred.GetComponent<Rigidbody>();
            rb.isKinematic = true;
        ...
        {
            Rigidbody rb;
            rb = initializedprojectedgreen.GetComponent<Rigidbody>();
            rb.isKinematic = true;
            rb.detectCollisions = false;
        }
        else
        ...
        {
            Rigidbody rb;
            rb = initializedprojectedblue.GetComponent<Rigidbody>();
            rb.isKinematic = true;
            rb.detectCollisions = false;
        }
        else
        ...
        position_plane.z = position_plane.z - 0.2f;
        ...
        if (moveObject2)
        {
            ...
            drop_pos2.y = drop_pos2.y + 0.15f; //new position to "drop" the object from
            ...
            if (current_object_to_move == 0)
            {
                ...
                if (execute2 == true)
                {
                    second_object.transform.position = drop_pos2;
                    targetplaced2 = true;
                    execute2 = false;
        ...
        if (moveObject3)
        {
            ...
            drop_pos3 = second_object.transform.position; //get the position of the first object 
            drop_pos3.y = drop_pos2.y + 0.15f; //new position to "drop" the object from
            ...
            if (current_object_to_move == 0)
            {
                ...
                if (execute3 == true)
                {
                    third_object.transform.position = drop_pos3;
                    execute3 = false;
                }
