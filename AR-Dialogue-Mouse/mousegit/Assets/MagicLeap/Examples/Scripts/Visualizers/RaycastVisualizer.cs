...
{
    /// <summary>
    /// Updates the transform and color on the Hit Position and Normal from the assigned object.
    /// </summary>
    public class RaycastVisualizer : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            ////Creates and writes new header for output file
            
            //System.IO.File.WriteAllText(@"C:\Users\craft\Documents\Unity Projects\EyeGaze-Output\EyeGazeVectors.txt", "\n\nTrial Vectors " + System.DateTime.Now.ToString("G") + " :\nRaycastOrigin, RaycastDirection, ResultPoint\n");

            //Starts input and sets controller to active controller
            //MLInput.Start();
        ...
        public void OnRaycastHit(MLWorldRays.MLWorldRaycastResultState state, RaycastHit result, float confidence)
        {
            if (state != MLWorldRays.MLWorldRaycastResultState.RequestFailed && state != MLWorldRays.MLWorldRaycastResultState.NoCollision)
            {
                ...
                //Logs Vectors when trigger is pressed
                //if(_controller.TriggerValue > 0.2f)
                //{
                //Debug.Log(_raycast.RayOrigin.ToString() + ", " + _raycast.RayDirection.ToString() + ", " + result.ToString());
                //System.IO.File.WriteAllText(@"C:\users\craft\Documents\Unity Projects\EyeGaze-Output\EyeGazeVectors.txt", _raycast.RayOrigin.ToString() + ", " + _raycast.RayDirection.ToString() + ", " + result.ToString() + "\n");
                //}
            ...
            else
            {
                ...
                //{
                // Debug.Log(_raycast.RayOrigin.ToString() + ", " + _raycast.RayDirection.ToString() + ", " + result.ToString());
                //System.IO.File.WriteAllText(@"C:\users\craft\Documents\Unity Projects\EyeGaze-Output\EyeGazeVectors.txt", _raycast.RayOrigin.ToString() + ", " + _raycast.RayDirection.ToString() + ", noTarget\n");
                //}
